    ...
    <DashboardLayout>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-zinc-200 pb-5">
          <div>
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-2xl font-extrabold text-black tracking-tight">{hackathon.title}</h1>
            ...
            <p className="text-zinc-600 text-xs mt-1 font-semibold">
        ...
        <div className="flex gap-1 border-b border-zinc-200">
          {tabs.map(tab => (
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2.5 text-xs font-bold capitalize border-b-2 transition-colors cursor-pointer ${
                activeTab === tab.id
            ...
            >
              ...
              {tab.count !== undefined && (
                <span className={`ml-0.5 px-1.5 py-0.5 rounded-full text-[10px] font-bold ${
                  activeTab === tab.id ? "bg-black text-white" : "bg-zinc-200 text-zinc-700"
                }`}>
                  {tab.count}
                </span>
              )}
      ...
      {assignSubModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
          <div className="bg-white border border-zinc-200 rounded-2xl p-6 w-full max-w-md shadow-xl space-y-4">
            <div className="flex items-center justify-between border-b border-zinc-200 pb-3">
              <div>
                <h3 className="text-base font-bold text-black">Assign Judges to Project</h3>
                <p className="text-xs text-zinc-500 truncate max-w-xs">{assignSubModal.projectName}</p>
              </div>
              <button onClick={() => setAssignSubModal(null)} className="text-zinc-500 hover:text-black p-1 cursor-pointer">
                <HiOutlineX />
              </button>
            </div>

            <p className="text-xs text-zinc-600">Select which judge(s) will be assigned to evaluate this project:</p>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs font-semibold text-black">{judge.name}</p>
            ...
            <div className="flex gap-3 justify-end pt-2 border-t border-zinc-200">
              <button onClick={() => setAssignSubModal(null)} className="btn-secondary text-xs px-4 py-2">Cancel</button>
              <button
      ...
      {rejectModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs">
          <div className="bg-white border border-zinc-200 rounded-2xl p-6 w-full max-w-md shadow-xl space-y-4">
            <h3 className="text-base font-bold text-black">Reject Registration</h3>
            <p className="text-zinc-600 text-sm">Optionally provide a reason — it will be sent to the team leader.</p>
            <textarea
              value={rejectReason}
      ...
      {deleteStep > 0 && hackathon && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4">
          <div className="bg-white border border-red-300 rounded-2xl p-6 w-full max-w-md shadow-xl space-y-4 relative">
            <button onClick={() => setDeleteStep(0)} className="absolute top-4 right-4 text-zinc-500 hover:text-black p-1 cursor-pointer">
              <HiOutlineX />
            </button>
            ...
            {deleteStep === 1 && (
              <div className="space-y-4">
                <div className="w-12 h-12 rounded-2xl bg-red-50 text-red-700 border border-red-200 flex items-center justify-center text-2xl mx-auto">
                  <HiOutlineTrash />
                </div>

                <div className="text-center space-y-1">
                  <h3 className="text-lg font-black text-black">Delete Hackathon?</h3>
                  <p className="text-xs text-red-600 font-bold uppercase tracking-wider">Step 1 of 2: First Confirmation</p>
                </div>

                <div className="bg-red-50 border border-red-200 p-4 rounded-xl text-xs text-zinc-700 space-y-2">
                  <p className="font-semibold text-black">Are you sure you want to delete <span className="text-red-600">"{hackathon.title}"</span>?</p>
            ...
            {deleteStep === 2 && (
              <div className="space-y-4">
                <div className="w-12 h-12 rounded-2xl bg-red-100 text-red-700 border border-red-300 flex items-center justify-center text-2xl mx-auto">
                  <HiOutlineTrash />
                </div>

                <div className="text-center space-y-1">
                  <h3 className="text-lg font-black text-black">Final Confirmation Required</h3>
                  <p className="text-xs text-red-600 font-bold uppercase tracking-wider">Step 2 of 2: Permanent Purge</p>
                </div>

                <div className="space-y-2">
                  <p className="text-xs text-zinc-700">
                    To confirm deletion, please type the exact title <strong className="text-amber-700">{hackathon.title}</strong> below:
                  </p>
                  <input
                    type="text"
                    value={confirmInput}
                    ...
                    className="input-field text-sm border-red-300 focus:border-red-500 font-semibold"
                  />
                </div>
