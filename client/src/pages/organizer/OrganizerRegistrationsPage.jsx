      ...
      <div className="space-y-6 max-w-5xl">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-200 pb-4">
          <div>
            <h1 className="text-2xl font-extrabold text-black tracking-tight flex items-center gap-2">
              <HiOutlineUserGroup className="text-black" /> Team Registrations
            </h1>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <select
            ...
            className="input-field sm:max-w-[160px] text-sm bg-white text-black border-zinc-300"
          >
        ...
        {loading ? (
          ...
          <div className="empty-state py-16 card">
            ...
            <p className="text-zinc-600 text-sm mt-2 font-semibold">Select a hackathon to view registrations</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="empty-state py-16 card">
            <HiOutlineFilter className="text-4xl text-zinc-600" />
            <p className="text-zinc-600 text-sm mt-2 font-semibold">No registrations match your filters</p>
          </div>
        ) : (
          <div className="card p-0 overflow-hidden bg-white border-zinc-200">
            <table className="data-table">
              ...
              <tbody>
                {filtered.map(r => (
                  <tr key={r._id}>
                    <td className="font-bold text-black">{r.team?.name}</td>
                    <td>
                      <div>
                        <p className="text-black text-sm font-semibold">{r.team?.leader?.name}</p>
                        <p className="text-zinc-600 text-xs">{r.team?.leader?.email}</p>
                      </div>
                    </td>
                    <td className="text-zinc-700 text-sm font-medium">{r.team?.members?.length || 0}</td>
      ...
      {rejectModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs">
          <div className="bg-white border border-zinc-200 rounded-2xl p-6 w-full max-w-md shadow-xl space-y-4">
            ...
            <p className="text-zinc-600 text-sm">Optionally provide a reason for the team leader.</p>
            <textarea
              value={rejectReason}
