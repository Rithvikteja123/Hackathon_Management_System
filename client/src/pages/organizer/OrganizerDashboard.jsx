    ...
    <DashboardLayout>
      <div className="space-y-5 max-w-5xl">
        ...
        <div className="flex items-center justify-between border-b border-zinc-200 pb-3">
          <div>
            <h1 className="text-xl font-extrabold text-black">Organizer Dashboard</h1>
            <p className="text-zinc-600 text-xs mt-0.5">Welcome back, <span className="text-black font-extrabold">{user?.name}</span>!</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {stats.map((s) => (
            <div key={s.label} className="bg-white border border-zinc-200 p-3.5 rounded-xl flex items-center justify-between shadow-xs">
              <div>
                <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider">{s.label}</p>
                <p className="text-lg font-black text-black mt-0.5">{loading ? "—" : s.value}</p>
              </div>
        ...
        <div className="grid md:grid-cols-3 gap-3">
          <Link to="/organizer/registrations" className="p-3.5 rounded-xl bg-white border border-zinc-200 hover:border-zinc-400 transition-colors flex items-center gap-3 group">
            <div className="w-8 h-8 rounded-lg bg-amber-50 text-amber-700 flex items-center justify-center text-sm flex-shrink-0">
              <HiOutlineUserGroup />
            </div>
            <div className="min-w-0">
              <p className="text-xs font-bold text-black group-hover:text-zinc-800 transition-colors">Manage Registrations</p>
              <p className="text-[11px] text-zinc-500 truncate">Approve or reject teams</p>
            </div>
            <HiArrowRight className="ml-auto text-zinc-400 group-hover:text-black text-xs flex-shrink-0" />
          </Link>

          <Link to="/organizer/submissions" className="p-3.5 rounded-xl bg-white border border-zinc-200 hover:border-zinc-400 transition-colors flex items-center gap-3 group">
            ...
            <div className="min-w-0">
              <p className="text-xs font-bold text-black group-hover:text-zinc-800 transition-colors">Review Submissions</p>
              <p className="text-[11px] text-zinc-500 truncate">Manage project entries</p>
            </div>
            <HiArrowRight className="ml-auto text-zinc-400 group-hover:text-black text-xs flex-shrink-0" />
          </Link>

          <Link to="/organizer/hackathons/create" className="p-3.5 rounded-xl bg-white border border-zinc-200 hover:border-zinc-400 transition-colors flex items-center gap-3 group">
            <div className="w-8 h-8 rounded-lg bg-violet-50 text-violet-700 flex items-center justify-center text-sm flex-shrink-0">
              <HiOutlinePlus />
            </div>
            <div className="min-w-0">
              <p className="text-xs font-bold text-black group-hover:text-zinc-800 transition-colors">Create Hackathon</p>
        ...
        <div className="bg-white border border-zinc-200 rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-200">
            <h3 className="font-bold text-black text-xs uppercase tracking-wider flex items-center gap-2">
              <RiTrophyLine className="text-amber-700 text-sm" /> My Hackathons
            </h3>
            <Link to="/organizer/hackathons" className="text-xs text-black hover:underline flex items-center gap-1 transition-colors font-semibold">
          ...
          {loading ? (
            ...
            <div className="empty-state py-10">
              <HiOutlineCollection className="text-3xl text-zinc-400" />
              <p className="text-zinc-600 text-xs mt-1">No hackathons created yet</p>
            ...
            <div className="divide-y divide-zinc-200">
              {hackathons.slice(0, 6).map(h => (
                <div key={h._id} className="flex items-center justify-between gap-3 px-4 py-3 hover:bg-zinc-50 transition-colors group">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-9 h-9 rounded-lg bg-zinc-100 overflow-hidden flex-shrink-0 border border-zinc-200">
                      {h.bannerImage
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="text-xs font-bold text-black truncate">{h.title}</p>
