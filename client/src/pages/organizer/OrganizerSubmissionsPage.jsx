      ...
      <div className="space-y-6 max-w-6xl">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-200 pb-4">
          <div>
            <h1 className="text-2xl font-extrabold text-black tracking-tight flex items-center gap-2">
              <HiOutlineDocumentText className="text-black" /> Project Submissions & Judge Assignment
            </h1>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <select
            ...
            className="input-field sm:max-w-xs text-sm bg-white text-black border-zinc-300"
          >
          ...
          <select
            ...
            className="input-field sm:max-w-[180px] text-sm bg-white text-black border-zinc-300"
          >
        ...
        {loading ? (
          ...
          <div className="empty-state py-16 card">
            ...
            <p className="text-zinc-600 text-sm mt-2 font-semibold">Select a hackathon to view submissions</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="empty-state py-16 card">
            <HiOutlineFilter className="text-4xl text-zinc-600" />
            <p className="text-zinc-600 text-sm mt-2 font-semibold">No submissions match your filters</p>
          </div>
        ) : (
          <div className="space-y-4">
            {filtered.map(s => {
              ...
              return (
                <div key={s._id} className="card space-y-4 bg-white border-zinc-200">
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="text-base font-bold text-black">{s.projectName}</h3>
                        <span className={`badge ${statusBadge[s.status] || "badge-gray"} capitalize text-[10px] font-bold`}>
                          {s.status?.replace(/_/g, " ")}
                        </span>
                      </div>
                      <p className="text-zinc-600 text-xs mt-0.5">
                        Team: <span className="text-black font-extrabold">{s.team?.name}</span>
          ...
          <div className="bg-white border border-zinc-200 rounded-2xl p-6 w-full max-w-md shadow-xl space-y-4">
            <div className="flex items-center justify-between border-b border-zinc-200 pb-3">
              <div>
                ...
                <p className="text-xs text-zinc-500 truncate max-w-xs">{assignModalSub.projectName}</p>
              </div>
              <button onClick={() => setAssignModalSub(null)} className="text-zinc-500 hover:text-black p-1 cursor-pointer">
                <HiOutlineX />
              </button>
            </div>

            <p className="text-xs text-zinc-600">Select which judge(s) will be assigned to evaluate this submission:</p>
            ...
            <div className="flex gap-3 justify-end pt-2 border-t border-zinc-200">
              <button onClick={() => setAssignModalSub(null)} className="btn-secondary text-xs px-4 py-2">Cancel</button>
              <button
