    ...
    <DashboardLayout>
      <div className="space-y-6 max-w-5xl">
        <div className="flex items-center justify-between border-b border-zinc-200 pb-4">
          <div>
            <h1 className="text-2xl font-extrabold text-black">My Hackathons</h1>
            <p className="text-zinc-600 text-xs mt-0.5">{hackathons.length} hackathon{hackathons.length !== 1 ? "s" : ""}</p>
          </div>
            ...
            {hackathons.map(h => (
              <div key={h._id} className="card flex flex-col md:flex-row md:items-center justify-between gap-4 border-zinc-800 hover:border-zinc-700 transition-all">
                ...
                <div className="flex items-center gap-4 min-w-0">
                  <div className="w-16 h-16 rounded-xl bg-zinc-100 overflow-hidden flex-shrink-0 border border-zinc-300">
                    {h.bannerImage
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="font-extrabold text-black text-base truncate">{h.title}</h3>
                    ...
                    <p className="text-xs text-zinc-600 font-semibold mt-0.5">{h.theme} · {h.mode}</p>
        ...
        {deleteStep > 0 && targetHackathon && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4">
            <div className="bg-white border border-red-300 rounded-2xl p-6 w-full max-w-md shadow-xl space-y-4 relative">
              ...
              <button onClick={closeDeleteFlow} className="absolute top-4 right-4 text-zinc-500 hover:text-black p-1 cursor-pointer">
                <HiOutlineX />
              </button>
              ...
              {deleteStep === 1 && (
                <div className="space-y-4">
                  ...
                  <div className="bg-red-50 border border-red-200 p-4 rounded-xl text-xs text-zinc-700 space-y-2">
                    <p className="font-semibold text-black">Are you sure you want to delete <span className="text-red-600">"{targetHackathon.title}"</span>?</p>
                  ...
                  <div className="flex gap-3 justify-end pt-2 border-t border-zinc-200">
                    <button onClick={closeDeleteFlow} className="btn-secondary text-xs px-4 py-2">Cancel</button>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <p className="text-xs text-zinc-700">
                      To confirm deletion, please type the exact title <strong className="text-amber-700">{targetHackathon.title}</strong> below:
                    </p>
                    <input
                      type="text"
                      value={confirmInput}
