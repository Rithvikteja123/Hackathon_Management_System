    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between border-b border-zinc-200 pb-3">
          <div>
            <h1 className="text-2xl font-extrabold text-black">Manage Users</h1>
            <p className="text-zinc-600 text-sm">{total} total users</p>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs">
          <div className="bg-white border border-zinc-200 rounded-2xl p-6 w-full max-w-md shadow-xl space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-black">Edit User</h3>
              <button onClick={() => setEditUser(null)} className="text-zinc-500 hover:text-black p-1 cursor-pointer"><HiOutlineX /></button>
            </div>

            {/* User Info */}
            <div className="flex items-center gap-3 p-3 bg-zinc-50 rounded-xl border border-zinc-200">
              <div className="w-10 h-10 rounded-full bg-black flex items-center justify-center font-bold text-white overflow-hidden">
                {editUser.avatar ? <img src={editUser.avatar} alt="" className="w-full h-full object-cover" /> : editUser.name?.[0]?.toUpperCase()}
              </div>
              <div>
                <p className="text-sm font-semibold text-black">{editUser.name}</p>
