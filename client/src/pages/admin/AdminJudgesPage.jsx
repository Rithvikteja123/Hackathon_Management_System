...
const AdminJudgesPage = () => {
  ...
  const [editRole, setEditRole] = useState("judge");
  ...
  const handleBlock = async (id, isBlocked) => {
    try {
      ...
      toast.success(isBlocked ? "Judge unblocked" : "Judge blocked");
      fetchUsers();
    } catch (e) {
  ...
  const handleDelete = async (id, name) => {
    if (!confirm(`Delete judge "${name}"? This action cannot be undone.`)) return;
    try {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      toast.success("Judge role updated successfully");
      setEditUser(null);
      fetchUsers();
    } catch (e) {
        ...
        <div className="flex items-center justify-between border-b border-zinc-200 pb-3">
          <div>
            <h1 className="text-xl font-extrabold text-black flex items-center gap-2">
              <HiOutlineScale className="text-black" /> Platform Judges
            </h1>
            <p className="text-zinc-600 text-xs mt-0.5">Manage all registered judges and project evaluators</p>
          </div>
            ...
            <table className="data-table text-xs">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={4} className="text-center text-zinc-600 py-10 font-semibold">No judges found</td></tr>
                ) : (
                  users.map(u => (
                    <tr key={u._id}>
                      <td>
