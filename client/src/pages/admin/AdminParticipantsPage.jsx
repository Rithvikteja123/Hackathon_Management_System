...
const AdminParticipantsPage = () => {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const res = await userAPI.getAll({ search, role: "participant", page, limit: 20 });
  ...
  const handleBlock = async (id, isBlocked) => {
    try {
      ...
      toast.success(isBlocked ? "Participant unblocked" : "Participant blocked");
      fetchUsers();
    } catch (e) {
  ...
  const handleDelete = async (id, name) => {
    if (!confirm(`Delete participant "${name}"? This action cannot be undone.`)) return;
    try {
      ...
      <div className="space-y-5 max-w-5xl">
        ...
        <div className="flex items-center justify-between border-b border-zinc-200 pb-3">
          <div>
            <h1 className="text-xl font-extrabold text-black flex items-center gap-2">
              <HiOutlineUsers className="text-black" /> Platform Participants
            </h1>
            <p className="text-zinc-600 text-xs mt-0.5">Manage all registered hackathon hackers and student participants</p>
          </div>
          <span className="badge badge-success text-[10px]">{total} Participants</span>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="input-field text-xs pl-9 py-2 bg-white text-black border-zinc-300"
            />
          </div>
        </div>

        {/* Table */}
        <div className="card p-0 overflow-hidden bg-white border-zinc-200">
            ...
            <table className="data-table text-xs">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={4} className="text-center text-zinc-600 py-10 font-semibold">No participants found</td></tr>
                ) : (
                  users.map(u => (
                    <tr key={u._id}>
                      <td>
                        <div className="flex items-center gap-2.5">
                          <div className="w-8 h-8 rounded-full bg-black text-white flex items-center justify-center font-bold text-xs overflow-hidden flex-shrink-0">
                            {u.avatar ? <img src={u.avatar} alt="" className="w-full h-full object-cover" /> : u.name?.[0]?.toUpperCase()}
                          </div>
                          <div>
                            <p className="font-bold text-black text-xs">{u.name}</p>
        ...
        {editUser && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
            <div className="bg-white border border-zinc-200 rounded-2xl p-5 w-full max-w-sm shadow-xl space-y-4 relative">
              <button onClick={() => setEditUser(null)} className="absolute top-4 right-4 text-zinc-500 hover:text-black p-1 cursor-pointer">
                <HiOutlineX />
              </button>
              <h3 className="font-bold text-black text-sm">Edit User Role — {editName}</h3>
              <div>
