        ...
        <div className="flex items-center justify-between border-b border-zinc-200 pb-3">
          <div>
            <h1 className="text-xl font-extrabold text-black flex items-center gap-2">
              <HiOutlineUserGroup className="text-black" /> Platform Teams
            </h1>
            <p className="text-zinc-600 text-xs mt-0.5">Overview of all participating teams across all hackathons</p>
          </div>
            ...
            <table className="data-table text-xs">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={6} className="text-center text-zinc-600 py-10 font-semibold">No teams found</td></tr>
                ) : (
                  filteredTeams.map((t) => (
                    <tr key={t._id}>
                      <td>
                        <p className="font-extrabold text-black text-xs">{t.name}</p>
                        <p className="text-[10px] text-zinc-500 font-mono">ID: {t._id?.slice(-6)}</p>
                      </td>
                      <td>
                        <p className="font-semibold text-black text-xs">{t.hackathon?.title || "Event"}</p>
