        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-200 pb-4">
          <div>
            <h1 className="text-2xl font-extrabold text-black tracking-tight flex items-center gap-2">
              <HiOutlineDocumentText className="text-black" /> Project Submission Portal
            </h1>
            <p className="text-zinc-600 text-xs mt-1">Submit or manage your team's hackathon project entry</p>
          </div>
        ...
        {selectedHackathon ? (
          loading ? (
            ...
            <motion.div
              ...
              className="card bg-amber-50 border-amber-200 text-amber-900 space-y-3"
            >
            ...
            <motion.div
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
              className="card space-y-5 border-emerald-300 bg-white"
            >
              {/* Success Banner */}
              <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center justify-between gap-3 flex-wrap">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-emerald-100 border border-emerald-300 flex items-center justify-center text-emerald-700 text-lg">
                    <HiOutlineCheckCircle />
                  </div>
                  <div>
                    <p className="text-xs font-bold text-emerald-800 uppercase tracking-wider">Project Successfully Submitted</p>
                    <p className="text-xs text-zinc-600">Team: <span className="text-black font-semibold">{team.name}</span></p>
                  </div>
                </div>
              ...
              <div className="space-y-4 pt-1">
                <div>
                  ...
                  <p className="text-lg font-extrabold text-black mt-0.5">{submission.projectName}</p>
                </div>
                ...
                <div className="flex gap-3 flex-wrap pt-1">
                  {submission.githubRepo && (
                    ...
                    >
                      <FaGithub className="text-zinc-600" /> GitHub Repository
                    </a>
                  )}
                  {submission.liveDemoUrl && (
                    <a
                      ...
                      className="btn-secondary text-xs px-3 py-1.5 flex items-center gap-1.5"
                    >
                      <HiOutlineExternalLink className="text-zinc-600" /> Live Demo
                    </a>
                  )}
                  {submission.demoVideoLink && (
                    <a
                      ...
                      className="btn-secondary text-xs px-3 py-1.5 flex items-center gap-1.5"
                    >
                      <HiOutlinePlay className="text-zinc-600" /> Demo Video
                    </a>
                  )}
                </div>
                ...
                {submission.solutionDescription && (
                  <div>
                    ...
                    <p className="text-xs text-zinc-700 leading-relaxed bg-zinc-50 p-3 rounded-xl border border-zinc-200 whitespace-pre-wrap">
                      {submission.solutionDescription}
                    </p>
                  </div>
                )}
            ...
            <motion.div
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
              className="card space-y-5 bg-white border-zinc-200"
            >
              {/* Form Status Bar */}
              <div className="flex items-center justify-between border-b border-zinc-200 pb-3 flex-wrap gap-2">
                <div>
                  ...
                  <p className="text-xs text-zinc-500">
                    Team: <span className="text-black font-semibold">{team.name}</span>
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  {submission && (
                    <button
                      ...
                      className="btn-ghost btn-sm text-xs text-zinc-600 flex items-center gap-1"
                    >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="input-field bg-white border-zinc-200 focus:border-black text-black text-xs"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="input-field bg-white border-zinc-200 focus:border-black text-black text-xs resize-none"
                    required
                  />
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    type="submit"
                    disabled={submitting}
                    className="btn-primary text-xs py-3 flex-1 flex items-center justify-center gap-2 shadow-xs"
                  >
                    {submitting ? (
                      "Saving..."
                    ) : submission ? (
          ...
          <div className="empty-state py-12">
            <HiOutlineDocumentText className="text-4xl text-zinc-400" />
            <p className="text-zinc-600 text-xs">Please select a hackathon from the dropdown above to view or submit your project.</p>
          </div>
        )}
      </div>
    </DashboardLayout>
