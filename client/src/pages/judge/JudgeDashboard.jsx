    ...
    <DashboardLayout>
      <div className="space-y-5 max-w-4xl">
        ...
        <div className="border-b border-zinc-200 pb-3">
          <h1 className="text-xl font-extrabold text-black">Judge Dashboard</h1>
          <p className="text-zinc-600 text-xs mt-0.5">
            Welcome, <span className="text-black font-bold">{user?.name}</span> · Ready to evaluate submissions?
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          <Link to="/judge/projects" className="p-3.5 rounded-xl bg-white border border-zinc-200 hover:border-zinc-400 transition-colors flex items-center gap-3 group">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <p className="text-xs font-bold text-black group-hover:text-zinc-800 transition-colors">Pending Reviews</p>
                {totalPendingCount > 0 && (
          ...
          <Link to="/judge/completed" className="p-3.5 rounded-xl bg-white border border-zinc-200 hover:border-zinc-400 transition-colors flex items-center gap-3 group">
            ...
            <div className="min-w-0">
              <p className="text-xs font-bold text-black group-hover:text-zinc-800 transition-colors">Completed Reviews ({reviews.length})</p>
        ...
        <div className="bg-white border border-zinc-200 rounded-xl p-4 space-y-3">
          <div className="flex items-center justify-between border-b border-zinc-200 pb-2.5">
            <h3 className="font-bold text-black text-xs uppercase tracking-wider flex items-center gap-2">
              <HiOutlineDocumentText className="text-purple-600 text-sm" /> My Assigned Team Submissions ({allAssignedSubmissions.length})
            </h3>
          ...
          {loading ? (
            ...
            <div className="empty-state py-6">
              ...
              <p className="text-zinc-600 text-xs mt-1">No team submissions assigned to you yet</p>
            </div>
          ) : (
                ...
                >
                  <div className="space-y-0.5 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h4 className="text-xs font-extrabold text-black truncate">{subItem.projectName}</h4>
                      {subItem.isCompleted ? (
                    ...
                    <p className="text-[11px] text-zinc-600">
                      Team: <span className="text-black font-bold">{subItem.teamName}</span>
                      <span className="text-zinc-400"> · </span>
                      Event: <span className="text-zinc-800">{subItem.hackathonTitle}</span>
                    </p>
                  </div>
