  ...
  return (
    <DashboardLayout>
      <div className="max-w-4xl space-y-6">
        ...
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 border-b border-zinc-200 pb-4">
          <div className="flex-1 min-w-0">
            <Link
              to={`/judge/projects?hackathon=${hackathonId}`}
              className="text-xs text-zinc-600 hover:text-black flex items-center gap-1 mb-2 transition-colors w-fit font-medium"
            >
              <HiOutlineArrowLeft /> Back to Assigned Projects
            </Link>
            <h1 className="text-xl font-extrabold text-black tracking-tight">{submission.projectName}</h1>
            <p className="text-zinc-600 text-xs mt-0.5">
              {hackathon.title} · Team: <span className="text-black font-bold">{submission.team?.name}</span>
            </p>
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {/* Score preview pill */}
            {existingReview && (
              <div className="bg-zinc-100 border border-zinc-300 rounded-xl px-4 py-2 text-center">
                <p className="text-xl font-black text-black">{existingReview.totalScore}</p>
                <p className="text-[10px] text-zinc-600">/{maxTotal} pts</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex gap-1 border-b border-zinc-200">
          {["project", "score"].map(tab => (
            <button
              ...
              className={`px-5 py-2.5 text-xs font-bold capitalize border-b-2 transition-colors cursor-pointer ${
                activeTab === tab
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
            ...
            <div className="card space-y-4">
              <div>
                ...
                <p className="text-zinc-800 text-sm leading-relaxed">{submission.problemStatement}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-1">Solution Description</p>
                <p className="text-zinc-800 text-sm leading-relaxed">{submission.solutionDescription}</p>
              </div>
              ...
              <div className="flex flex-wrap gap-2 pt-1 border-t border-zinc-200">
                ...
                {submission.demoVideoLink && (
                  <a href={submission.demoVideoLink} target="_blank" rel="noopener noreferrer"
                    className="text-xs px-3 py-2 flex items-center gap-1.5 rounded-lg border border-red-500/30 text-red-600 hover:bg-red-500/10 transition-colors">
                  ...
                  {submission.screenshots.map((sc, i) => (
                    <a key={i} href={sc.url} target="_blank" rel="noopener noreferrer">
                      <img src={sc.url} alt={`Screenshot ${i + 1}`}
                        className="rounded-xl object-cover w-full h-32 border border-zinc-300 hover:border-black transition-colors cursor-pointer" />
                    </a>
                  ))}
                </div>
              </div>
            ...
            <button
              ...
              className="btn-primary w-full py-3 justify-center text-sm flex items-center gap-2 cursor-pointer"
            >
              <HiOutlineStar />
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="card bg-white border-zinc-200">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3 className="text-sm font-bold text-black">Score Summary</h3>
                    <p className="text-xs text-zinc-600 mt-0.5">
                      {existingReview ? "Update your existing scores" : "Score each criterion — scores update the live leaderboard"}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-3xl font-black text-black">{totalScore}</p>
                    <p className="text-xs text-zinc-600">/ {maxTotal} pts</p>
                  </div>
                </div>
                ...
                <div className="w-full bg-zinc-200 rounded-full h-2.5 overflow-hidden border border-zinc-300">
                  <motion.div
                    className={`h-full rounded-full transition-all ${
                      pct >= 80 ? "bg-black" :
                      pct >= 60 ? "bg-zinc-800" :
                      pct >= 40 ? "bg-amber-600" : "bg-red-600"
                    }`}
                    initial={{ width: 0 }}
                    animate={{ width: `${pct}%` }}
                ...
                <p className="text-[10px] text-zinc-600 mt-1 text-right font-semibold">{pct}% of maximum</p>
              </div>
              ...
              {scores.length === 0 ? (
                <div className="empty-state py-8 card">
                  ...
                  <p className="text-zinc-700 text-sm mt-2 font-semibold">No judging criteria set for this hackathon</p>
                  <p className="text-zinc-500 text-xs">The organizer needs to add criteria before scoring can begin</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {scores.map((s, i) => (
                    <div key={s.criterion} className="card space-y-3 bg-white border-zinc-200">
                      <div className="flex items-center justify-between">
                        <div>
                          <label className="text-sm font-bold text-black">{s.criterion}</label>
                          {s.description && (
                            <p className="text-xs text-zinc-600 mt-0.5">{s.description}</p>
                          )}
                        </div>
                        <div className="px-3 py-1.5 rounded-xl border border-zinc-300 bg-zinc-100 text-black text-xs font-extrabold">
                          {s.marks} / {s.maxMarks}
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <input
                          ...
                          className="flex-1 h-2 appearance-none rounded-full cursor-pointer accent-black"
                          style={{ background: `linear-gradient(to right, #000000 ${(s.marks/s.maxMarks)*100}%, #e4e4e7 ${(s.marks/s.maxMarks)*100}%)` }}
                        />
                        <input
                          type="number"
                          min={0}
                          ...
                          value={s.marks}
                          onChange={e => handleScoreChange(i, e.target.value)}
                          className="input-field w-20 text-center text-sm font-bold text-black bg-white border-zinc-300"
                        />
                      </div>
                    </div>
                  ))}
              ...
              {/* Written Feedback */}
              <div className="card space-y-3 bg-white border-zinc-200">
                <label className="text-sm font-bold text-black">Written Feedback</label>
                <p className="text-xs text-zinc-600">Constructive feedback helps teams improve. Teams will be able to read this.</p>
                <textarea
                  value={feedback}
                  ...
                  className="input-field resize-none text-sm text-black bg-white border-zinc-300"
                />
              </div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="btn-secondary flex items-center gap-1.5 text-sm px-4 cursor-pointer"
                >
                ...
                <button
                  ...
                  className="btn-primary flex-1 justify-center text-sm py-3 flex items-center gap-2 cursor-pointer"
                >
                  {submitting ? (
