...
const HackathonCard = ({ hackathon }) => (
  ...
  >
    ...
    <div className="p-5 flex flex-col flex-1">
      <h3 className="font-bold text-black mb-1 text-base leading-snug group-hover:text-zinc-700 transition-colors line-clamp-1">
        {hackathon.title}
      </h3>
      <p className="text-xs text-zinc-600 font-medium mb-2">{hackathon.theme}</p>
      <p className="text-sm text-zinc-600 line-clamp-2 mb-4 flex-1">{hackathon.description}</p>
  ...
  const pageContent = (
    <div className={isAuthenticated ? "max-w-7xl mx-auto space-y-6" : "pt-24 pb-16 px-4 max-w-7xl mx-auto"}>
      ...
      <div className="mb-6">
        ...
        <p className="text-zinc-600 mt-1">{total} hackathons available</p>
      </div>

      {/* Search + Filters */}
      <div className="flex flex-col md:flex-row gap-3 mb-8">
        <div className="relative flex-1">
          <HiOutlineSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400" />
          <input
            value={search}
        ...
        {hasFilters && (
          <button onClick={clearFilters} className="btn-ghost btn-sm text-zinc-600 flex items-center gap-1">
            <HiOutlineX /> Clear
          </button>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state py-20 card">
          ...
          <p className="text-black font-semibold">No hackathons found</p>
          <p className="text-zinc-600 text-sm mt-1">Try resetting your filters or search query</p>
          ...
          {Array.from({ length: pages }, (_, i) => i + 1).map((p) => (
            <button
              ...
              className={`w-8 h-8 rounded-lg text-sm font-semibold transition-all cursor-pointer ${
                page === p ? "bg-black text-white font-bold" : "bg-zinc-100 text-zinc-700 hover:bg-zinc-200"
              }`}
            >
              {p}
            </button>
