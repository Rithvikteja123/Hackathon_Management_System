  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900 selection:bg-black/10 selection:text-black">
      ...
      <section className="relative pt-32 pb-20 md:pt-40 md:pb-32 overflow-hidden">
        {/* Ambient Spotlights */}
        <div className="blob w-[550px] h-[550px] bg-black/5 -top-20 left-1/2 -translate-x-1/2" />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-12 gap-12 items-center">
            ...
            >
              {/* Pill Tag */}
              <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-zinc-100 border border-zinc-200 text-zinc-800 text-xs font-bold uppercase tracking-wider mb-6 shadow-xs">
                <HiOutlineSparkles className="text-black text-sm animate-pulse" />
                Production MERN Hackathon Platform
              </div>

              {/* Headline (Compact & Refined) */}
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold tracking-tight leading-[1.2] mb-4 text-black">
                Run Hackathons with <br className="hidden sm:inline" />
                <span className="gradient-text-glow">Monochrome SaaS Precision.</span>
              </h1>

              {/* Subtitle */}
              <p className="text-base sm:text-lg text-zinc-600 max-w-xl leading-relaxed mb-8">
              ...
              {/* Feature Highlights */}
              <div className="flex flex-wrap items-center gap-6 pt-4 border-t border-zinc-200 text-xs font-medium text-zinc-600">
                <div className="flex items-center gap-2">
                  <HiOutlineCheckCircle className="text-black text-base" />
            ...
            >
              <div className="relative rounded-2xl border border-zinc-200 bg-white p-2.5 shadow-xl group overflow-hidden">
                <div className="relative rounded-xl overflow-hidden aspect-[4/3]">
                  <img
                    src="/hackathon-hero.png"
                    alt="CoBuild — Live Hackathon Arena"
                    className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
                  
                  {/* Floating Overlay Badge */}
                  <div className="absolute bottom-4 left-4 right-4 p-3.5 rounded-xl bg-white/90 backdrop-blur-md border border-zinc-200 flex items-center justify-between">
                    <div>
                      <p className="text-xs font-bold text-black">Live Hackathon Arena</p>
                      <p className="text-[11px] text-zinc-600">500+ Developers Competing</p>
                    </div>
      ...
      {/* =========================================================
         3. LIVE STATS STRIP
         ========================================================= */}
      <section className="border-y border-zinc-200 bg-zinc-50 py-10 px-4">
        <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6">
          {stats.map((s) => (
            <div key={s.label} className="text-center">
              <p className="text-3xl sm:text-4xl font-extrabold text-black tracking-tight gradient-text">{s.value}</p>
              <p className="text-sm font-semibold text-zinc-800 mt-1">{s.label}</p>
      ...
      <section className="py-14 px-4 sm:px-6 max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((f, i) => (
            ...
            >
              <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${f.color} border border-zinc-200 flex items-center justify-center mb-5`}>
                <f.icon className={`text-2xl ${f.iconColor}`} />
              </div>
              <h3 className="text-lg font-bold text-black mb-2 group-hover:text-zinc-700 transition-colors">{f.title}</h3>
      ...
      {/* =========================================================
         ...
         ========================================================= */}
      <section className="py-24 px-4 sm:px-6 bg-zinc-50 border-y border-zinc-200">
          ...
          <div className="flex flex-wrap justify-center gap-2 mb-12">
            {Object.keys(roleShowcase).map((r) => (
              <button
                ...
                className={`px-5 py-2.5 rounded-xl font-bold text-sm capitalize transition-all cursor-pointer ${
                  activeRole === r
            ...
            >
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <h3 className="text-3xl font-extrabold text-black">{currentRole.title}</h3>
                  <p className="text-zinc-600 mt-2 text-base leading-relaxed">{currentRole.desc}</p>
                </div>

                <div className="space-y-3">
                  {currentRole.perks.map((p) => (
                    <div key={p} className="flex items-center gap-3">
                      <HiOutlineCheckCircle className="text-black text-xl flex-shrink-0" />
                      <span className="text-zinc-800 text-sm font-medium">{p}</span>
                    </div>
                  ))}
                </div>
              ...
              <div className="card-glass border-zinc-200 bg-white p-6 space-y-4">
                <div className="flex items-center justify-between border-b border-zinc-200 pb-3">
                  <span className="text-xs font-mono text-zinc-500">ROLE DEMO PREVIEW</span>
                  <span className="badge badge-primary">ACTIVE ROUTE</span>
                </div>

                {activeRole === "participant" && (
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between"><span className="text-zinc-600">Team Name:</span><strong className="text-black">{currentRole.preview.teamName}</strong></div>
                    <div className="flex justify-between"><span className="text-zinc-600">Project:</span><strong className="text-zinc-800">{currentRole.preview.project}</strong></div>
                    <div className="flex justify-between"><span className="text-zinc-600">Status:</span><span className="badge badge-success">{currentRole.preview.status}</span></div>
                ...
                {activeRole === "organizer" && (
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between"><span className="text-zinc-600">Hackathon:</span><strong className="text-black">{currentRole.preview.title}</strong></div>
                    <div className="flex justify-between"><span className="text-zinc-600">Registrations:</span><strong className="text-zinc-800">{currentRole.preview.registered}</strong></div>
                    <div className="flex justify-between"><span className="text-zinc-600">Submissions:</span><strong className="text-zinc-800">{currentRole.preview.submissions}</strong></div>
                  </div>
                )}

                {activeRole === "judge" && (
                  <div className="space-y-3 text-sm">
                    <p className="font-semibold text-zinc-800">{currentRole.preview.evaluating}</p>
                    {currentRole.preview.criteria.map(c => (
                      <div key={c.name} className="flex justify-between text-xs border-b border-zinc-200 pb-1">
                        <span className="text-zinc-600">{c.name}</span>
                        <strong className="text-black">{c.score}</strong>
                      </div>
                    ))}
                    <div className="flex justify-between text-base font-bold pt-2">
                      <span className="text-zinc-800">Total Awarded:</span>
                ...
                {activeRole === "admin" && (
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between"><span className="text-zinc-600">Platform Users:</span><strong className="text-black">{currentRole.preview.users}</strong></div>
                    <div className="flex justify-between"><span className="text-zinc-600">Total Hackathons:</span><strong className="text-zinc-800">{currentRole.preview.hackathons}</strong></div>
                    <div className="flex justify-between"><span className="text-zinc-600">Health:</span><span className="badge badge-success">{currentRole.preview.status}</span></div>
                  </div>
                )}
              </div>
            </motion.div>
      ...
      <section className="py-14 px-4 sm:px-6 max-w-6xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-black tracking-tight">Frequently Asked Questions</h2>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-6 items-stretch">
          ...
          <div className="lg:col-span-7 space-y-3">
            {faqs.map((faq, i) => (
              <div key={i} className="card p-0 overflow-hidden border-zinc-200 bg-white">
                <button
                  onClick={() => setActiveFaq(activeFaq === i ? null : i)}
                  className="w-full text-left px-5 py-3.5 flex items-center justify-between font-semibold text-xs sm:text-sm text-zinc-800 hover:text-black transition-colors cursor-pointer"
                >
                  <span>{faq.q}</span>
                  <HiOutlineChevronDown className={`text-base flex-shrink-0 transition-transform ${activeFaq === i ? "rotate-180 text-black" : "text-zinc-400"}`} />
                </button>
                {activeFaq === i && (
                  <div className="px-5 pb-3.5 text-xs text-zinc-600 leading-relaxed border-t border-zinc-200 pt-3 bg-zinc-50">
                    {faq.a}
                  </div>
                )}
              </div>
          ...
          <div className="lg:col-span-5 flex flex-col">
            <div className="card-glass py-6 px-6 text-center relative overflow-hidden border-zinc-200 bg-white flex-1 flex flex-col justify-center items-center">
              <div className="blob w-40 h-40 bg-black/5 -top-8 -left-8" />
      ...
      <footer className="border-t border-zinc-200 bg-white py-12 px-4">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-6 text-sm text-zinc-600">
            <Link to="/hackathons" className="hover:text-black transition-colors">Hackathons</Link>
            ...
            <Link to="/signup" className="hover:text-black transition-colors">Sign Up</Link>
          </div>
        </div>
      </footer>
    </div>
