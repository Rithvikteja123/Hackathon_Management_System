...
const HackathonDetailsPage = () => {
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900">
      ...
      <div className="pt-16">
        {/* Banner */}
        <div className="relative h-72 md:h-96 bg-gradient-to-br from-zinc-100 to-zinc-200 overflow-hidden">
          {h.bannerImage ? (
            <img src={h.bannerImage} alt={h.title} className="w-full h-full object-cover" />
          ) : (
            <div className="w-full h-full flex items-center justify-center">
              <RiTrophyLine className="text-8xl text-zinc-300" />
            </div>
          )}
          ...
          <div className="absolute bottom-0 left-0 right-0 max-w-5xl mx-auto px-4 pb-6">
            ...
            <p className="text-zinc-200 text-sm mt-1">
              by <span className="text-white font-semibold">{h.organizer?.name}</span>
            </p>
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto px-4 py-8">
          <div className="grid lg:grid-cols-3 gap-8">
              ...
              >
                {activeTab === "overview" && (
                  <div className="prose max-w-none">
                    <p className="text-zinc-700 leading-relaxed whitespace-pre-line">{h.description}</p>
                    {h.assignedJudges?.length > 0 && (
                      <div className="mt-6">
                        <h3 className="text-black font-semibold mb-3">Judges</h3>
                        <div className="flex flex-wrap gap-3">
                          {h.assignedJudges.map(j => (
                            <div key={j._id} className="flex items-center gap-2 bg-zinc-100 border border-zinc-200 rounded-lg px-3 py-2">
                              <div className="w-7 h-7 rounded-full bg-black text-white flex items-center justify-center text-xs font-bold">
                                {j.name?.[0]}
                              </div>
                              <span className="text-sm text-zinc-800">{j.name}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                  ...
                  <div>
                    {h.prizePool ? (
                      <div className="text-center py-8">
                        <RiTrophyLine className="text-5xl text-amber-500 mx-auto mb-3" />
                        <p className="text-2xl font-bold gradient-text">{h.prizePool}</p>
                        <p className="text-zinc-600 text-sm mt-1">Total Prize Pool</p>
                        {h.winners?.length > 0 && (
                          <div className="mt-6 space-y-3">
                            {h.winners.map(w => (
                              <div key={w.position} className="flex items-center gap-3 justify-center">
                                ...
                                <span className="text-zinc-800">{w.prize}</span>
                              </div>
                            ))}
                          </div>
                        )}
            ...
            <div className="space-y-4">
              {/* Registration CTA */}
              <div className="card">
                <h3 className="font-semibold text-black mb-4">Registration</h3>
                <div className="space-y-2.5 text-sm mb-4">
                  <div className="flex items-center gap-2 text-zinc-600">
                    <HiOutlineCalendar className="text-zinc-700 flex-shrink-0" />
                    <span>Deadline: <span className="text-zinc-900">{format(new Date(h.registrationDeadline), "MMM d, yyyy")}</span></span>
                  </div>
                  <div className="flex items-center gap-2 text-zinc-600">
                    <HiOutlineUsers className="text-zinc-700 flex-shrink-0" />
                    <span>Team: <span className="text-zinc-900">{h.minTeamSize}–{h.maxTeamSize} members</span></span>
                  </div>
                  {h.venue && (
                    <div className="flex items-center gap-2 text-zinc-600">
                      <HiOutlineLocationMarker className="text-zinc-700 flex-shrink-0" />
                      <span className="text-zinc-900">{h.venue}</span>
                    </div>
                  )}
                </div>
                ...
                {!h.registrationOpen || regDeadlinePast ? (
                  ...
                  <div className="card-glass p-3 text-center space-y-1 bg-zinc-50 border-zinc-200">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={e.label} className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-zinc-900 font-medium">{format(new Date(e.date), "MMM d, yyyy")}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {(h.website || h.contactEmail) && (
                <div className="card">
                  ...
                  {h.website && (
                    <a href={h.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-zinc-800 hover:underline mb-2">
                      <HiOutlineGlobe /> {h.website}
                    </a>
                  )}
                  {h.contactEmail && (
                    <a href={`mailto:${h.contactEmail}`} className="flex items-center gap-2 text-sm text-zinc-800 hover:underline">
